...
export async function runScraper(bot: any, chatId: string) {
  ...
  try {
    ...
    // 1. Initial Selection
    await page.goto('https://sede.administracionespublicas.gob.es/icpplus/index.html', { waitUntil: 'networkidle' });
    ...
    if (content.includes("En este momento no hay citas disponibles")) {
      ...
      await Log.create({ lastName: fullName, lastNationality: data.country, success: true, foundOffices: offices });
